...
public class TouristRoutingModuleWithChanceToUseRouter implements RoutingModule {
    ...
    public List<? extends PlanElement> calcRoute(Facility fromFacility, Facility toFacility, double departureTime, Person person) {
        ...
        if (!toFacility.getLinkId().equals(fromFacility.getLinkId())) {
            ...
            boolean randomWalker = true;
            if (person.getAttributes().getAttribute("subpopulation").equals("loc")) {
                double randomWalkProb = 1.0;
                try {
                    randomWalkProb = (double) person.getAttributes().getAttribute("randomWalkProb");
                }catch (Exception e){

                }
                randomWalker = chance > randomWalkProb;
            }
            LeastCostPathCalculator.Path path = null;
            if (randomWalker)
                path = pathSimulator.simulatePath(fromLink, toLink, departureTime, person, null);
            else {
            ...
            NetworkRoute
                    route = this.populationFactory.getRouteFactories().createRoute(NetworkRoute.class, fromFacility.getLinkId(), toFacility.getLinkId());
    ...
    /**
     * Finds the path between two nodes, assuming constant walking speed of 1m/s
     */
