...
public class TouristRoutingModuleWithDifferentTourists implements RoutingModule {
    ...
    public TouristRoutingModuleWithDifferentTourists(Network network, PopulationFactory populationFactory) {
    ...
    public List<? extends PlanElement> calcRoute(Facility fromFacility, Facility toFacility, double departureTime, Person person) {
        ...
        if (!toFacility.getLinkId().equals(fromFacility.getLinkId())) {
            ...
            if (person.getAttributes().getAttribute("subpopulation").equals("tur")){
                LeastCostPathCalculator.Path path = pathSimulatorLandmark.simulatePath(fromLink, toLink, departureTime, person, null);
                if (path == null) {
                ...
                LeastCostPathCalculator.Path path = pathSimulatorRandom.simulatePath(fromLink, toLink, departureTime, person, null);
                if (path == null) {
                    throw new RuntimeException("No route found on inverted network from link "
                            + fromFacility.getLinkId() + " to link " + toFacility.getLinkId() + ".");
                }
